...
class FWIForward(nn.Module):
    '''
    Forward modeling with hard-coded, self-contained experiment scenarios.
    The 'ctx' should only contain universal physical parameters.
    '''
    ...
    def forward(self, v, scenario, client_idx=None, num_clients=None):
        if self.normalize:
            v = self.v_denorm_func(v)
        
        # Create a working copy of the context for this run
        run_ctx = copy.deepcopy(self.ctx)

        # --- Define all experiment geometries here ---
        ...
        # --- Partition based on client_idx or run the full (global) model ---
        if client_idx is None:
            # SERVER-SIDE (GLOBAL) RUN
