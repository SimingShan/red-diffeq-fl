...
class FWIForward(nn.Module):
    ...
    def forward(self, v, client_idx=None, num_clients=None, scenario=None):
        ...
        if client_idx is None:
            ...
            # --- Scenarios for 2 clients (requires a scenario string) ---
            if num_clients == 2:
                if scenario == '2A':
                    if client_idx == 0:
            ...
            # --- The only scenario for 3 clients (does not use a scenario string) ---
            elif num_clients == 3:
                if client_idx == 0:
                    run_ctx['sx'] = self.ctx['sx'][:2]  # 2 sources
                    run_ctx['gx'] = self.ctx['gx'][0:17] # First 23 receivers
                elif client_idx == 1:
                    run_ctx['sx'] = self.ctx['sx'][1:4]  # 3 sources
                    run_ctx['gx'] = self.ctx['gx'][17:53] # Next 24 receivers
                else: # client_idx == 2
                    run_ctx['sx'] = self.ctx['sx'][3:]  # 2 sources
                    run_ctx['gx'] = self.ctx['gx'][53:70] # Final 23 receivers
                
                run_ctx['ns'] = len(run_ctx['sx'])
            
            # --- No other client counts are supported ---
            else:
                raise NotImplementedError(f"Partitioning logic for {num_clients} clients is not defined.")
