# This script implement the forward solver for 5 clients given client index
import numpy as np
import torch
...
class FWIForward(nn.Module):
    ...
    def forward(self, v, client_idx=None):
        if self.normalize:
            v = self.v_denorm_func(v)
            
        if client_idx is not None:            
            run_ctx = copy.deepcopy(self.ctx)
            run_ctx['sx'] = self.ctx['sx'][client_idx:client_idx+1]
            run_ctx['ns'] = len(run_ctx['sx'])
            run_ctx['gx'] = self.ctx['gx']
        else:
            run_ctx = self.ctx
